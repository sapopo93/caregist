...
async def get_pipeline_health(conn: asyncpg.Connection) -> dict[str, Any]:
    ...
    units = await conn.fetchrow(
        ...
        SELECT COUNT(*) AS location_rows,
               ...
               COUNT(DISTINCT provider_id) FILTER (
                 WHERE UPPER(status) = 'ACTIVE' AND provider_id IS NOT NULL AND provider_id != ''
               ) AS active_provider_organisations,
               COUNT(DISTINCT provider_id) FILTER (
                 WHERE UPPER(status) = 'ACTIVE' AND provider_id IS NOT NULL AND provider_id != ''
                   AND group_name IS NOT NULL AND BTRIM(group_name) != ''
               ) AS grouped_provider_organisations,
               COUNT(DISTINCT BTRIM(group_name)) FILTER (
                 WHERE UPPER(status) = 'ACTIVE' AND group_name IS NOT NULL AND BTRIM(group_name) != ''
    ...
    if pipeline_runs_exists:
        latest_run = await conn.fetchrow(
            ...
            FROM pipeline_runs
            WHERE run_type IN ('incremental', 'feed_cycle', 'reconciliation')
    ...
    if pipeline_runs_exists:
        latest_source_run = await conn.fetchrow(
            """
            SELECT run_type, source_uri, source_published_at, source_retrieved_at,
            ...
            WHERE status = 'completed' AND source_published_at IS NOT NULL
              AND run_type IN ('incremental', 'reconciliation')
    ...
    source_fresh = bool(
        ...
        and timedelta(0) <= source_age <= SOURCE_FRESHNESS_SLA
    )
    ...
    # A quiet period with no new registrations is valid. Processing freshness
    # is proved by a recent completed pipeline run; event timestamps describe
    # the data observed and must not manufacture a failure when there was no event.
    feed_fresh = recent_run_ok

    source_details = {
        "source": "Care Quality Commission public directory",
        "sourceRunType": latest_source_run["run_type"] if latest_source_run else None,
    ...
    checks.append({"name": "cqc_source_watermark", "ok": source_fresh and counts_reconciled, "details": source_details})
    ...
    checks.append(
        {
            "name": "new_registration_feed_processing",
            "ok": feed_fresh,
            "details": {
                ...
                "latestEventIsInformational": True,
                "recentPipelineRunRequired": True,
            },
        }
    )

    readiness_ok = pipeline_runs_exists and trusted_event_ledger_exists
    freshness_ok = source_fresh and counts_reconciled and feed_fresh and recent_run_ok
